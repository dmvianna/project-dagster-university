...
def trips_by_week(
    ...
    preview_df = aggregate.head().to_markdown(index=False)

    try:
        # If the file already exists, append to it, but replace the
        # existing month's data

        existing = pd.read_csv(constants.TRIPS_BY_WEEK_FILE_PATH)
        existing = existing[existing["period"] != period_to_fetch]
        existing = pd.concat([existing, aggregate]).sort_values(by="period")  # pyright: ignore [reportCallIssue]
    ...
    return dg.MaterializeResult(
        metadata={
            "row_count": dg.MetadataValue.int(aggregate.shape[0]),
            "preview": dg.MetadataValue.md(preview_df),  # pyright: ignore [reportArgumentType]
        }
    )
